...
//            System.out.println(wolf[i][0] + "\t" + wolf[i][1] + "\t" + fox[i]);
    ...
    private static void dijkWolf() {
        ...
        while (!pq.isEmpty()) {
            ...
            if(wolf[0][cnode] < ctime && wolf[1][cnode] < ctime) {
                continue;
            }
            ...
            for (Node next : graph[cnode]) {
                ...
                if (current.speed == 4) { // 느릴 때
                    if (wolf[0][nnode] > wolf[1][cnode] + ntime * 4) {
                        wolf[0][nnode] = wolf[1][cnode] + ntime * 4;
                        pq.add(new Node(nnode, wolf[0][nnode], 1));
                    }
                } else { // 존빠열
                    if (wolf[1][nnode] > wolf[0][cnode] + ntime * 1) {
                        wolf[1][nnode] = wolf[0][cnode] + ntime * 1;
